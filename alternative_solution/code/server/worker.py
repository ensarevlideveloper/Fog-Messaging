...
cycles = 0
#Dictionary of requests from the client for which a reply was sent but no acknowledgement received
...
while True:
    ...
    if socks.get(worker) == zmq.POLLIN:
        #  Get message
        #  - 4-part envelope + index + content -> request
        ...
        if len(frames) == 4 and frames[3] == SIGNAL_EOL:
            #Go into server crashes/overloads to test
            if (test_mode == True):
            ...
            worker.send_multipart(frames)
            #Add to dictionary to check for replies without acknowledgement
